...
## Mandatory Localization & Zero-Latency Performance Rules (中国国内网络与零延迟规则)

> [!CRITICAL]
> **NEVER import external Google Fonts (`fonts.googleapis.com` / `fonts.gstatic.com`) or foreign font CDNs directly in application CSS.**
>
> In domestic (China / mainland) network environments, requesting `fonts.googleapis.com` can cause high network latency, timeouts, proxy blockages, or render-blocking delays.
>
> **Mandatory Font Strategy**:
> 1. Use zero-latency, offline-first system font stacks:
>    - **Sans-Serif**: `font-family: Inter, system-ui, -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, "Helvetica Neue", Arial, sans-serif;`
>    - **Monospace**: `font-family: ui-monospace, SFMono-Regular, Consolas, "Liberation Mono", Menlo, Monaco, "Fira Code", monospace;`
> 2. If a specific web font is required by design, either bundle it locally inside the application asset directory or use domestic mirror CDNs. Never leave live blocking `@import url('https://fonts.googleapis.com...')` calls in production CSS.
