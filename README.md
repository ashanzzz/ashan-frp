...
`ashan-frp` 当前主线是 **前端 UI + API 服务 + Job Runner + FRPC Runtime Manager**。旧 shell 脚本已不再作为推荐入口，只保留为历史迁移痕迹。
...
│   ▼                                                         │
│ 前端 UI                                                     │
│ （仪表盘 / 节点 / 隧道 / 网站映射 / 作业 / 设置 / 日志）     │
│   ▼                                                         │
│ API 服务                                                    │
│ （鉴权 / 校验 / 入库 / 查询 / SSE 推送）                    │
│   ▼                                                         │
│ 数据库                                                      │
│ （accounts / nodes / tunnels / jobs / sync_state / ...）    │
│   ▼                                                         │
│ Job Runner                                                  │
│ （领取 job、执行、重试、写事件）                             │
│   ▼                                                         │
│ FRPC Runtime Manager                                        │
│ （start / stop / restart / reload / health_check /          │
│  switch_node / recover）                                    │
│   ▼                                                         │
│ 外部运行时                                                  │
│ （FRP / 1Panel / Docker / Cloudflare DNS）                  │
└─────────────────────────────────────────────────────────────┘
```
...
> 下面这些命令来自旧 shell 工作流，仅用于历史说明，不再是当前主线入口。
