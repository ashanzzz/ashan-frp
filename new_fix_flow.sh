...
LOG_DIR="/mnt/user/Hdd_Disk_Share/脚本日志/chmlfrp"
LOG_FILE=""
...
FRPC_CONFIG_PATH="/mnt/user/appdata/frpc/frpc.toml"
...
init_paths() {
  ...
  TUNNEL_LIST_RAW="$LOG_DIR/chmlfrp隧道列表_raw.json"
  TUNNEL_LIST_FORMATTED="$LOG_DIR/chmlfrp隧道列表_格式化.json"

  DNS_RAW_FILE="$LOG_DIR/cloudflare_dns_raw.json"
  DNS_FORMATTED_FILE="$LOG_DIR/cloudflare_dns_格式化.json"

  NODE_FILE="$LOG_DIR/chmlfrp节点筛选.txt"
  EXEMPT_NODE_FILE="$LOG_DIR/chmlfrp豁免列表.txt"
  FIXED_TS_FILE="$LOG_DIR/.fixed_tunnel.last_ts"
  ONLINE_TEST_FILE="$LOG_DIR/chmlfrp-frpc在线测试.txt"
}
...
log_rotate_and_print() {
  ...
  if [ "$line_count" -gt "$MAX_LOG_LINES" ]; then
    if tail -n "$MAX_LOG_LINES" "$LOG_FILE" > "${LOG_FILE}.tmp" 2>/dev/null; then
      ...
      echo "[$(date '+%Y-%m-%d %H:%M:%S')][INFO] 日志已截断，仅保留最后 $MAX_LOG_LINES 行" >> "$LOG_FILE"
    fi
  fi
}
...
check_frpc_logs_for_config_mismatch() {
  info "检查 frpc 容器日志中是否存在配置错误（客户端代理参数错误）..."
  ...
  local last_logs
  last_logs=$(docker logs --tail 5 "$FRPC_DOCKER_NAME" 2>/dev/null)
...
check_source_ts() {
  ...
  if check_frpc_logs_for_config_mismatch; then
    frpc_error=1
    warning "frpc 日志检测到 [客户端代理参数错误]，将继续执行修复流程"
  fi
  ...
  if [[ "$frpc_error" -eq 0 \
    ...
    success "固定隧道/豁免文件 mtime+hash 均未变化，在线测试正常且 frpc 日志无错误 ➜ 无需执行修复流程，脚本直接退出。"
    exit 0
  fi

  # 7) 打印触发原因（表示需要进入修复流程）
  info "本次将继续执行修复流程，触发原因如下："
  ...
  if [[ "$frpc_error" -eq 1 ]]; then
    info " - 检测到 frpc 日志中存在 [客户端代理参数错误]"
  else
    info " - frpc 日志正常，无配置错误信息"
  fi

  success "综上：满足上述至少一条条件，继续执行测速和修复流程。"
}
...
read_userdetail_id() {
  ...
  if [ ! -f "$TUNNEL_USERDATA_FILE" ]; then
    warning "用户详情文件不存在 => $TUNNEL_USERDATA_FILE => 无法解析用户 id"
    return 1
  fi
  if ! jq empty "$TUNNEL_USERDATA_FILE" >/dev/null 2>&1; then
    warning "用户详情 JSON 不合法 => $TUNNEL_USERDATA_FILE"
    return 1
  fi
...
select_best_node() {
  ...
  for ((i = 0; i < total; i++)); do
    ...
    url="http://cf-v2.uapis.cn/nodeinfo?token=${token}&node=${name_encoded}"
...
select_node_by_name() {
  ...
  if [ ! -f "$NODE_FILE" ] || ! jq empty "$NODE_FILE" >/dev/null 2>&1; then
    error "节点文件不存在或非法 => $NODE_FILE"
    return 1
  fi

  local idx
  idx=$(jq -r --arg name "$name" '.nodes | to_entries[] | select(.value["节点名称"]==$name) | .key' "$NODE_FILE" | head -n 1)
  ...
  ip_from_file=$(jq -r ".nodes[$idx][\"节点本地IPv4\"] // \"\"" "$NODE_FILE")
...
prefer_node_from_frpc() {
  ...
  if [ ! -f "$NODE_FILE" ] || ! jq empty "$NODE_FILE" >/dev/null 2>&1; then
    warning "prefer_node_from_frpc: 节点文件不存在或非法 => $NODE_FILE"
    return 1
  fi
  ...
  local name
  name=$(jq -r ".nodes[$idx][\"节点名称\"] // \"\"" "$NODE_FILE")
  if [ -z "$name" ]; then
...
tunnel_api_create() {
  ...
  if $DRY_RUN; then
    info "[DRY_RUN] 调用 create_tunnel: $json_payload"
...
tunnel_api_delete() {
  ...
  local base_url="https://cf-v1.uapis.cn/api/deletetl.php"
  local safe_token="${token:0:4}***"
  local url="${base_url}?token=${token}&nodeid=${nodeid}&userid=${userid}"

  if $DRY_RUN; then
    info "[DRY_RUN][DELETE-TUNNEL] GET ${base_url}?token=${safe_token}&nodeid=${nodeid}&userid=${userid}"
    echo '{"code":200,"error":"DRY_RUN"}'
    return 0
  fi

  info "[DELETE-TUNNEL][REQUEST][GET] ${base_url}?token=${safe_token}&nodeid=${nodeid}&userid=${userid}"

  local resp
  resp=$(curl -k -sS "$url" || echo "")
  ...
  info "[DELETE-TUNNEL][RESPONSE] nodeid=${nodeid} => $resp"

  local code msg
  code=$(echo "$resp" | jq -r '.code // 0')
  msg=$(echo "$resp"  | jq -r '.error // ""')

  if [ "$code" -eq 200 ]; then
    return 0
  fi

  if [ "$code" -eq 400 ] && echo "$msg" | grep -q "此隧道不属于你"; then
...
tunnel_get_fixed() {
  ...
  while IFS= read -r line; do
    ...
    jq --argjson x "$item" '. + [$x]' "$TEMP_FILE_FIXED" \
...
tunnel_fetch_list_local() {
  ...
  error "多次尝试获取隧道列表失败，视为当前无隧道"
  echo '{"data":[]}' > "$TUNNEL_LIST_RAW"
  return 1
}
...
tunnel_format_list() {
  ...
  if [[ ! -f "$TUNNEL_LIST_RAW" ]] || [[ ! -s "$TUNNEL_LIST_RAW" ]]; then
    ...
    echo "[]" > "$TUNNEL_LIST_FORMATTED"
    return 0
  fi

  # 情况 2：JSON 解析失败
  local data
  if ! data=$(jq '.data // []' "$TUNNEL_LIST_RAW" 2>/dev/null); then
    info "当前 ChmlFrp 隧道列表为空 (0 条)"
    echo "[]" > "$TUNNEL_LIST_FORMATTED"
    return 0
  fi
  ...
  while IFS= read -r item; do
    ...
    jq --argjson r "$new_entry" '. + [$r]' "$TUNNEL_LIST_FORMATTED" \
...
dns_fetch_list_local() {
  ...
  while [ $attempt -lt $max_retries ]; do
    ...
    if [ "$ok" = "true" ]; then
      echo "$resp" | jq '.result' > "$DNS_RAW_FILE"
...
dns_format_list() {
  ...
  # 情况 3：数组长度为 0
  if jq -e 'length==0' "$DNS_RAW_FILE" >/dev/null 2>&1; then
    info "当前 Cloudflare DNS 列表为空 (0 条)"
    echo "[]" > "$DNS_FORMATTED_FILE"
    return 0
  fi
...
perform_force_delete_all() {
  ...
  for ((round=1; round<=MAX_ROUNDS; round++)); do
    ...
    while IFS= read -r item; do
      ...
      if tunnel_api_delete "$CHMLFRP_USERID" "$id" >/dev/null; then
        ((TUNNELS_DELETED++))
        ((deleted_this_round++))
  ...
  if [ -f "$DNS_FORMATTED_FILE" ]; then
    ...
    while IFS=$'\t' read -r cname rid; do
      ...
      local in_fixed
      in_fixed=$(jq -r --arg c "$cname" 'map((.dns_domain_cname // .name) == $c) | any' "$TEMP_FILE_FIXED" 2>/dev/null || echo "false")
    ...
    done < <(jq -r '.[] | [.dns_domain_cname, .id] | @tsv' "$DNS_FORMATTED_FILE")
...
clean_invalid_resources() {
  ...
  if [ -f "$TUNNEL_LIST_FORMATTED" ] && [ -f "$TEMP_FILE_FIXED" ]; then
    while IFS=$'\t' read -r name tid; do
      ...
      local in_fixed
      in_fixed=$(jq -r --arg n "$name" 'map(select(.name == $n)) | length' "$TEMP_FILE_FIXED")
      if [ "$in_fixed" -eq 0 ]; then
        info "删除未在固定配置中的隧道 [$name] (id=$tid)"
        tunnel_api_delete "$CHMLFRP_USERID" "$tid" >/dev/null
      else
        debug "隧道 [$name] 存在于固定配置，保留"
      fi
    done < <(jq -r '.[] | [.name, .id] | @tsv' "$TUNNEL_LIST_FORMATTED")
  fi
  ...
  if [ -f "$DNS_FORMATTED_FILE" ] && [ -f "$TEMP_FILE_FIXED" ]; then
    while IFS=$'\t' read -r cname rid; do
      ...
      in_fixed=$(jq -r --arg c "$cname" '
        map((.dns_domain_cname // .name) == $c) | any
      ' "$TEMP_FILE_FIXED")
      if [ "$in_fixed" != "true" ]; then
...
tunnel_create_from_current_state() {
  ...
  while IFS= read -r item; do
    local name ip local_port rport ttype cname
    name=$(echo "$item"       | jq -r '.name // ""')
...
update_frpc_docker() {
  ...
  if docker ps -a --format '{{.Names}}' | grep -qw "$FRPC_DOCKER_NAME"; then
    info "检测到旧容器 [$FRPC_DOCKER_NAME]，删除并重建"
    docker rm -f "$FRPC_DOCKER_NAME" >/dev/null 2>&1 || warning "删除旧容器失败（忽略）"
  fi
...
main() {
  ...
  local fixed_count
  fixed_count=$(jq 'length' "$TEMP_FILE_FIXED" 2>/dev/null || echo 0)
  ...
  if ! perform_force_delete_all; then
    warning "删除非豁免资源过程中出现错误，将继续尝试重建"
  fi
