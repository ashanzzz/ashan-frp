...
const RECOVERY_COMMANDS = {
  local: './ashan-frp admin reset-password',
  docker: 'docker compose exec -it ashan-frp /app/ashan-frp admin reset-password',
};
...
function loginPanel() {
  return `<div class="login-card"><div class="login-intro"><div class="eyebrow">安全登录</div><h3>进入 Ashan FRP 运营台</h3><p>使用唯一管理员账号登录后，系统会加载可访问的数据和操作入口。</p></div><form id="login-form" class="login-form"><label for="login-username"><span>用户名</span><input id="login-username" name="username" autocomplete="username" placeholder="请输入管理员用户名" value="${esc(STATE.loginUsername)}" /></label><label for="login-password"><span>密码</span><input id="login-password" name="password" type="password" autocomplete="current-password" placeholder="请输入密码" value="${esc(STATE.loginPassword)}" /></label><div class="login-actions"><button id="login-btn" type="submit">登录运营台</button><button class="secondary" type="button" id="reload-btn">刷新服务状态</button></div><div class="login-help"><button class="link-button" type="button" id="forgot-password-btn">忘记密码？</button><span>仅有服务器或容器终端权限时可恢复唯一管理员</span></div></form></div>`;
}
