...
function appShell() {
  ...
  const loginText = auth ? `Logged in ? ${esc(auth.display_name || auth.login_name || auth.id || 'unknown')}` : STATE.sessionMode === 'anonymous' ? 'Anonymous / read-only' : 'Login state unknown';
