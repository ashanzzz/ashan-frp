...
func (h *DashboardHandler) Dashboard(c *gin.Context) {
	...
	c.JSON(http.StatusOK, domain.ResponseEnvelope{Data: domain.DashboardData{
		Version: domain.VersionInfo{Version: h.cfg.Version, Engine: "gin-gorm-sqlite", AppName: h.cfg.AppName, Status: "healthy"},
		Health:  domain.HealthInfo{Status: "healthy", Tunnels: len(tunnels), Jobs: len(jobs)},
		Tunnels: tunnels, Jobs: jobs, Settings: dto, RecentAuditLogs: auditLogs,
	}})
}
...
func (h *DashboardHandler) GetAuditLogs(c *gin.Context) {
	...
	if logs == nil { logs = []domain.AuditLog{} }
...
func (b *sseBroker) Unsubscribe(channel string, ch chan domain.Event) {
	...
	for i, sub := range subs {
		if sub == ch { b.subscribers[channel] = append(subs[:i], subs[i+1:]...); close(ch); return }
	}
}
...
func (b *sseBroker) Publish(evt domain.Event) {
	...
	if len(b.history) > 200 { b.history = b.history[len(b.history)-200:] }
	subs := make([]chan domain.Event, 0)
	if list, ok := b.subscribers[evt.Channel]; ok { subs = append(subs, list...) }
	if list, ok := b.subscribers[""]; ok { subs = append(subs, list...) }
	b.mu.Unlock()
	for _, ch := range subs { select { case ch <- evt: default: } }
}
...
func (b *sseBroker) History(channel, afterCursor string, limit int) []domain.Event {
	...
	start := 0
	if afterCursor != "" {
		for i, evt := range b.history { if evt.Cursor == afterCursor { start = i + 1; break } }
	}
	out := make([]domain.Event, 0, limit)
	for i := start; i < len(b.history); i++ {
		evt := b.history[i]
		if channel != "" && evt.Channel != channel { continue }
		out = append(out, evt)
...
func PublishEvent(evt domain.Event) {
	if evt.Cursor == "" { evt.Cursor = domain.NewID("evt") }
	if evt.CreatedAt.IsZero() { evt.CreatedAt = time.Now().UTC() }
	if evt.SchemaVersion == 0 { evt.SchemaVersion = 1 }
	broker.Publish(evt)
}

func (h *DashboardHandler) EventsStream(c *gin.Context) {
	channel := c.Query("channel"); cursor := c.Query("cursor")
	c.Header("Content-Type", "text/event-stream; charset=utf-8")
	c.Header("Cache-Control", "no-cache")
	c.Header("Connection", "keep-alive")
	c.Header("X-Accel-Buffering", "no")
	flusher, ok := c.Writer.(http.Flusher)
	if !ok { c.JSON(http.StatusInternalServerError, domain.ResponseEnvelope{Error: &domain.APIError{Code: "SSE_UNSUPPORTED", Message: "Streaming not supported"}}); return }
	for _, evt := range broker.History(channel, cursor, 100) { writeSSE(c.Writer, flusher, evt) }
	...
	for {
		...
		case evt, ok := <-sub: if !ok { return }; writeSSE(c.Writer, flusher, evt)
		case <-heartbeat.C: c.Writer.WriteString(": ping\n\n"); flusher.Flush()
		}
	}
}
