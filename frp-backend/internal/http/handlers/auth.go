...
func (h *AuthHandler) Login(c *gin.Context) {
	...
	if acc.LockedUntil != nil && time.Now().Before(*acc.LockedUntil) {
		c.JSON(http.StatusTooManyRequests, domain.ResponseEnvelope{Error: &domain.APIError{Code: "ACCOUNT_LOCKED", Message: "Account temporarily locked"}})
		return
	}
...
func (h *AuthHandler) Logout(c *gin.Context) {
	...
	if cookie != "" {
		if t, _ := h.repo.FindAuthTokenByHash(security.HashToken(cookie)); t != nil {
...
func (h *AuthHandler) Session(c *gin.Context) {
	...
	if err != nil || cookie == "" {
		c.JSON(http.StatusOK, domain.ResponseEnvelope{Data: map[string]any{"authenticated": false}})
		return
	}
	token, err := h.repo.FindAuthTokenByHash(security.HashToken(cookie))
	if err != nil || token == nil || !token.IsValid() {
		c.JSON(http.StatusOK, domain.ResponseEnvelope{Data: map[string]any{"authenticated": false}})
		return
	}
	_ = h.repo.TouchAuthToken(token.ID)
	account, err := h.repo.FindAccountByID(token.AccountID)
	if err != nil || account == nil {
		c.JSON(http.StatusOK, domain.ResponseEnvelope{Data: map[string]any{"authenticated": false}})
		return
	}
	c.JSON(http.StatusOK, domain.ResponseEnvelope{Data: map[string]any{"authenticated": true, "account": domain.AccountAuth{ID: account.ID, LoginName: account.LoginName, Role: account.Role}}})
}
...
func (h *AuthHandler) ChangePassword(c *gin.Context) {
	...
	if !security.VerifyPassword(req.OldPassword, acc.PasswordHash) {
		c.JSON(http.StatusBadRequest, domain.ResponseEnvelope{Error: &domain.APIError{Code: "INVALID_PASSWORD", Message: "Current password is incorrect"}})
		return
	}
	hash, err := security.HashPassword(req.NewPassword)
	if err != nil {
		c.JSON(http.StatusInternalServerError, domain.ResponseEnvelope{Error: &domain.APIError{Code: "INTERNAL", Message: err.Error()}})
		return
	}
