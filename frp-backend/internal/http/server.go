...
func (s *Server) routes() {
	...
	uiFS, _ := fs.Sub(uiDist, "dist")
	r.GET("/ui/*filepath", func(c *gin.Context) {
		path := c.Param("filepath")
		if path == "/" || path == "" {
			path = "/index.html"
		}
		f, err := uiFS.Open(path[1:])
		if err == nil {
			f.Close()
...
var docsPage = []byte(` + "`" + `<!doctype html><html lang="zh-CN"><head><meta charset="utf-8"><title>Ashan FRP API Docs</title><style>body{font-family:system-ui;margin:32px;background:#0d0e12;color:#e3e4e8}a{color:#4a9eed}pre{background:#16171f;padding:16px;border-radius:8px;overflow:auto}</style></head><body><h1>Ashan FRP API Docs</h1><p>OpenAPI: <a href="/api/v1/openapi.json">/api/v1/openapi.json</a></p><pre id="spec">loading...</pre><script>fetch("/api/v1/openapi.json").then(r=>r.text()).then(t=>document.getElementById("spec").textContent=t)</script></body></html>` + "`" + `)
