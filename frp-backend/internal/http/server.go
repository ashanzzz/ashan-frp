...
func (s *Server) routes() {
	...
	api.Use(middleware.AuthMiddleware(s.repo))
	{
		...
		api.GET("/tunnels/failover-pool", tunnelH.ListFailoverPool)
		...
		api.POST("/settings/integrations/cloudflare/configure", settingsH.ConfigureCloudflare)
		...
		api.POST("/nodes/speedtest-all", nodeH.SpeedTestAll)
		api.POST("/nodes/:id/speedtest", nodeH.SpeedTest)
		api.POST("/nodes/:id/preferred-pool", nodeH.SetPreferredPool)
