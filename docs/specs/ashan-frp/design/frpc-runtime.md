...
- 旧 shell 脚本时代的 `chmlfrp.sh` / `new_fix_flow.sh` 仍保留为历史入口，但不再是推荐主线
- 后端 scaffold、job runner、1Panel adapter、website sync、FRPC Runtime Manager 已经落地
- 前端页面和交互设计文档已存在，前端壳在持续完善
...
1. **继续完善前端代码壳**
2. **继续加固内置 frpc runtime**
3. **继续收束切换/自愈策略和文档**

原因：

- 现在后端基础已经有了，前端仍有收尾空间。
- frpc runtime 已落地，但还需要把节点切换/恢复链路继续收紧。
- 先把“看得见、点得到、能观察”的壳子补全，再收尾 runtime 和文档更稳。
